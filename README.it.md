...
- [Pagina Impostazioni](#pagina-impostazioni)
- [Manutenzione: riavvio camere via SSH](#manutenzione-riavvio-camere-via-ssh)
- [Personalizzazioni facili](#personalizzazioni-facili)
- [Versione e changelog](#versione-e-changelog)
- [Crediti](#crediti)
- [Licenza](#licenza)
...
- **Gestione automatica del fuso orario** — conversione dall'orologio
  ...
  ora legale/solare.
- **Pagina Impostazioni** — modifica tutti i parametri di `config.json`
  (camere incluse) da browser, con switch Basic/Advanced.
- **Manutenzione da remoto** — riavvia ogni camera via SSH con un click,
...
- **Pulsante di ricarica per singola tile** (icona circolare in alto a
  destra su ogni diretta): utile quando una camera va in errore o resta
  bloccata mentre le altre funzionano bene — ferma e riavvia solo quello
  stream, senza toccare gli altri né dover ricaricare l'intera pagina.
...
Bottone con icona ingranaggio ⚙ in fondo alla sidebar (`/settings`):
permette di modificare tutti i parametri di `config.json` — camere incluse
(aggiungere, rimuovere, modificare host/credenziali/icona/diretta) — senza
mai toccare il file a mano.

- **Switch Basic / Advanced** in cima alla pagina (default: Basic). In
  Basic sono visibili solo camere, fuso orario e Livewall — l'essenziale
  per la gestione quotidiana. Passando ad Advanced compaiono anche
  indicizzazione/cache, diretta HLS, HTTPS, e il blocco Manutenzione (vedi
  sotto). La scelta viene ricordata nel browser.
- Le **password lasciate vuote non vengono modificate** — resta il valore
  già salvato (anche se cifrato da `encrypt_config.py`).
- Il blocco `auth` (utente/hash password di accesso) **non è mai
  modificabile da qui** — resta gestito solo da `set_password.py` da
  terminale, per sicurezza.
- Dopo il salvataggio, il server tenta un riavvio automatico
  (`sudo systemctl restart yicam-timeline`, funziona se gira come
  servizio) e la pagina si ricarica da sola dopo qualche secondo. Se non
  gira come servizio, il file viene comunque salvato ma il riavvio va
  fatto a mano.

## Manutenzione: riavvio camere via SSH

Nel blocco **Manutenzione** delle impostazioni avanzate (`/settings`,
switch su Advanced), un pulsante "Riavvia" per ciascuna camera esegue un
riavvio da remoto via SSH, usando le **stesse credenziali FTP** già
configurate per quella camera.

- Il comando eseguito è `busybox reboot -f` (verificato funzionante su
  yi-hack-MStar; il comando `reboot` semplice a volte non basta a seconda
  del firmware).
- Richiede una conferma esplicita prima di procedere (popup del browser),
  per evitare riavvii accidentali.
- Se la porta SSH della camera non è la 22 standard, aggiungi
  `"ssh_port": <numero>` a quella camera in `config.json` (campo
  facoltativo, non esposto nella UI).

**Riavvio programmato**: per un riavvio schedulato (es. ogni notte), la
soluzione più pulita è il **crontab integrato in yi-hack stesso**
(Settings → Expert → Crontab nella pagina web della camera), non questo
pulsante. Esempio, riavvio ogni lunedì/mercoledì/venerdì alle 12:15:
```
15 12 * * 1,3,5 busybox reboot -f
```
Girando sulla camera stessa, funziona anche se il server di questo
progetto è spento o irraggiungibile.
...
Il numero di versione corrente compare in fondo alla pagina Impostazioni
(`/settings`), insieme alla data di rilascio — è anche un link cliccabile
che porta a `/changelog`, dove da questa versione in poi viene registrata
ogni modifica successiva. La numerazione segue uno schema libero
`MAJOR.MINOR.PATCH` (es. `0.9.20`), aggiornata manualmente in
`changelog.py` ad ogni release.
